 ...
 */
import {
  CampaignCreationExecutionEvidence, CampaignCreationBatch,
...
import { assertSpProviderMoneyScope, spMoneyNumber } from './sp-creation-money.js';
import type {
...
function requestItem(
  ...
  checkedIdentity: (nodeId: string) => string,
...
function evidenceIdentity(plan: CampaignCreationPlanV2, evidence: CampaignCreationExecutionEvidence, committed: boolean) {
  const nodes = new Map(plan.nodes.map((entry) => [entry.nodeId, entry]));
...
function compileSpCreationCall(
  ...
  const { kind, item } = requestItem(plan, node, evidenceIdentity(plan, evidence, committed));
...
/** A retry keeps its full source graph, but authorizes only its exact reviewed remaining nodes. */
export function prepareSpCreationBatchCall(raw: CampaignCreationBatch, nodeId: string,
  hasher: CampaignCreationSha256Hasher): SpCreationCompiledCall {
  const batch = CampaignCreationBatch.parse(raw);
  const plan = requireCampaignCreationDispatchInputs(verifyCampaignCreationPlanFingerprints(batch.plan, hasher));
  assertSpProviderMoneyScope(plan.providerScope);
  const node = plan.nodes.find((row) => row.nodeId === nodeId);
  if (!node || node.effect !== 'irreversible_create' || !batch.nodes.some((row) => row.nodeId === nodeId)) refuse('node_outside_batch');
  const checkedIdentity = (dependencyId: string): string => {
    const product = batch.productChecks.find((row) => row.nodeId === dependencyId);
    if (product) return product.providerEntityId;
    const inherited = batch.lineage?.inheritedResources.find((row) => row.nodeId === dependencyId);
    if (inherited) return inherited.providerEntityId;
    const parent = batch.nodes.find((row) => row.nodeId === dependencyId);
    if (parent?.observation?.observation !== 'observed' || parent.observation.providerEntityId === null) return refuse('dependency_not_observed');
    return parent.observation.providerEntityId;
  };
  const { kind, item } = requestItem(plan, node, checkedIdentity);
  const endpoint = SP_WRITE_ENDPOINTS[kind];
  const body = JSON.stringify({ [endpoint.requestKey]: [item] });
  const context = [batch.id, batch.draftId, batch.draftRevision, batch.actorId, batch.lineage,
    plan.id, plan.fingerprint, plan.providerScope, 'POST', endpoint.path, endpoint.mediaType];
  const position = Object.freeze({ requestIndex: 0, nodeId, nodeFingerprint: node.fingerprint,
    requestDigest: digest(JSON.stringify(['openspell.creation-batch-node.v1', ...context, item]), hasher) });
  return Object.freeze({ kind, method: 'POST', path: endpoint.path, mediaType: endpoint.mediaType,
    body, providerScope: Object.freeze({ ...plan.providerScope }), positions: Object.freeze([position]) as SpCreationCompiledCall['positions'],
    requestDigest: digest(JSON.stringify(['openspell.creation-batch-request.v1', ...context, position, body]), hasher) });
}
