...
export function compareSpCreationResource(kind: SpWriteKind, expected: Record<string, unknown>, row: JsonObject): Comparison {
...
export function decodeSpCreationReadback(
  ...
  try {
    ...
    return compareSpCreationResource(call.kind, expected, row);
