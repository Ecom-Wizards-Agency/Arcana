...
import {
  ...
  CampaignCreationBatch, CampaignBuilderCheck,
...
export function creationBatch(source = plan()): CampaignCreationBatch {
  const admittedAt = new Date(Date.parse(source.frozenAt) + 1000).toISOString();
  return CampaignCreationBatch.parse({ id: id(290), draftId: id(291), draftRevision: 2, actorId: id(292),
    plan: source, admittedAt, expiresAt: source.expiresAt, environmentGateVersion: id(293), profileGrantVersion: id(294),
    lineage: null, productChecks: source.nodes.flatMap((node) => node.kind === 'eligibility.require_product'
      ? [{ nodeId: node.nodeId, providerEntityId: node.payload.asin, observedAt: admittedAt }] : []),
    validation: { planFingerprint: source.fingerprint, recipeFingerprint: zero, checkedAt: admittedAt,
      checks: CampaignBuilderCheck.shape.id.options.map((id) => ({ id, label: id, source: 'Synthetic measured check',
        status: 'passed', blocking: false, currentValue: 'Synthetic value', requiredAction: '' })) },
    nodes: source.nodes.filter((node) => node.effect === 'irreversible_create').map((node) => ({ nodeId: node.nodeId,
      nodeFingerprint: node.fingerprint, intent: null, result: null, observation: null, refusal: null })) });
}
