...
it('binds restore selection and read instants to the immutable forward preview fingerprint',()=>{
  ...
  const source={...forward.source,restoreProposal:{kind:'restore_proposal' as const,sourceBatchId:forward.source.applyBatchId,sourceRowIds:[row.sourceRowId],rows:[row]}};
