...
describe('frozen SP preview evidence', () => {
  it('binds recommendation method identity, trace and setting sources without rewriting historical bytes', () => {
    const original = fixture();
    const method = { methodId: 'synthetic-method', methodVersion: 'synthetic.1',
      traceSha256: 'c'.repeat(64), settingSourcesSha256: 'd'.repeat(64) };
    const withMethod = { ...original, provenance: { ...original.provenance,
      rows: [{ ...original.provenance.rows[0]!, method }] } };
    const before = serializeSpWritePreviewProvenance(withMethod);
    expect(before).not.toBe(serializeSpWritePreviewProvenance(original));
    expect(serializeSpWritePreviewProvenance(JSON.parse(JSON.stringify(original))))
      .toBe(serializeSpWritePreviewProvenance(original));
    for (const change of [{ methodId: 'other-method' }, { methodVersion: 'synthetic.2' },
      { traceSha256: 'e'.repeat(64) }, { settingSourcesSha256: 'f'.repeat(64) }]) {
      expect(serializeSpWritePreviewProvenance({ ...withMethod, provenance: { ...withMethod.provenance,
        rows: [{ ...withMethod.provenance.rows[0]!, method: { ...method, ...change } }] } })).not.toBe(before);
    }
    expect(SpWritePreviewEvidence.safeParse({ ...withMethod, provenance: { ...withMethod.provenance,
      rows: [{ ...withMethod.provenance.rows[0]!, method: { ...method, traceSha256: 'unverified' } }] } }).success).toBe(false);
  });
