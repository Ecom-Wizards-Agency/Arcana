...
/** Saved history URLs also accept legacy numeric/UUID boundaries for either source.
 * A read boundary need not identify an existing entry; emitted identities stay strict.
 */
export const TimeMachineReadCursor = TimeMachineCursor.extend({
  id: z.union([
    TimeMachineEntryId,
    z.string().regex(new RegExp('^(?:change|apply):(?:[0-9]+|' + uuidPattern + ')$', 'i'))
      .transform((value) => value.toLowerCase()),
  ]),
});
export type TimeMachineReadCursor = z.infer<typeof TimeMachineReadCursor>;
