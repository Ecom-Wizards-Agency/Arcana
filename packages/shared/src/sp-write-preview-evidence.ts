...
/** Frozen identifiers, independent of the currently installed method registry.
 * Hashes cover the exact PostgreSQL JSONB text read from recommendation inputs.
 * Missing method evidence is readable history, but cannot receive a new approval.
 */
export const SpWriteRecommendationMethod = z.object({
  methodId: z.string().min(1),
  methodVersion: z.string().min(1),
  traceSha256: SpWriteSha256,
  settingSourcesSha256: SpWriteSha256,
}).strict();
export type SpWriteRecommendationMethod = z.infer<typeof SpWriteRecommendationMethod>;
...
export const SpWritePreviewEvidence = z.object({
  ...
  provenance: z.object({
    ...
    rows: z.array(z.object({
      ...
      method: SpWriteRecommendationMethod.optional(),
