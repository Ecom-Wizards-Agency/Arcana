...
import { TimeMachineCursor, TimeMachineReadCursor, TimeMachineNativeWrite, compareTimeMachineCursors } from './time-machine-writes.js';
...
describe('native Time Machine contracts', () => {
  ...
  it('accepts saved legacy read boundaries without widening emitted entry identities', () => {
    for (const boundary of [`change:${id('0')}`, 'apply:42', 'change:0']) {
      const cursor = { observedAt: '2000-01-01T00:00:00.000Z', id: boundary };
      expect(TimeMachineReadCursor.parse(cursor)).toEqual({ ...cursor, observedAt: '2000-01-01T00:00:00.000000Z' });
      expect(TimeMachineCursor.safeParse(cursor).success).toBe(false);
    }
    const native = { observedAt: at, id: `write:${id('2')}:${id('13')}:keyword.bid` };
    expect(TimeMachineReadCursor.parse(native)).toEqual(TimeMachineCursor.parse(native));
    for (const observedAt of ['2026-02-30T12:00:00Z', '0000-01-01T00:00:00Z', '2026-09-05T12:00:00.1234567Z']) {
      expect(TimeMachineReadCursor.safeParse({ ...native, observedAt }).success).toBe(false);
    }
    expect(TimeMachineReadCursor.safeParse({ ...native, id: 'unrecognized:42' }).success).toBe(false);
  });
});
