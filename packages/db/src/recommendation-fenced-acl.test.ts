...
const repair = await readFile(fileURLToPath(new URL(
  '../../../supabase/migrations/20260915100000_recommendation_fenced_function_acl.sql',
  import.meta.url,
)), 'utf8');
...
describe.skipIf(!available)('managed recommendation function ACL repair', () => {
  ...
  async function asManagedRepair(withAdmin = false) {
    await sql`begin`;
    try {
      // Roles are cluster-wide. Keep fixture grants uncommitted so migrations
      // in parallel disposable databases cannot observe the synthetic edges.
      if (withAdmin) {
      ...
      const result = await sql.unsafe(repair);
      await sql`reset session authorization`;
      expect((await snapshot())?.['memberships']).toEqual(before?.['memberships']);
      if (withAdmin) {
        await sql.unsafe(`revoke openspell_recommendation_executor, openspell_recommendation_worker
          from ${operator} granted by current_user`);
      }
      await sql`commit`;
      return result;
    } catch (error) {
  ...
  it('rolls back both ACL edits and temporary authority on a postflight failure', async () => {
    ...
    const before = await snapshot();
    await expect(asManagedRepair(true)).rejects.toThrow('ACL repair postflight failed');
    expect(await snapshot()).toEqual(before);
  });
});
