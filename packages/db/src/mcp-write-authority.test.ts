...
const migration = fileURLToPath(new URL('../../../supabase/migrations/20260915050000_mcp_write_delegation_mode.sql', import.meta.url));
...
describe.skipIf(!available)('MCP write authority migration boundary', () => {
  it('requires a commit before consuming the enum and leaves authority and execution empty', async () => {
    const database = await createTestDatabase('mcp_mode', {
      throughMigration: '20260915040000_recommendation_proposal_revisions.sql', applyFixture: false,
    });
    try {
