...
it('returns the real appended event when creation has a later recorded timestamp', async () => {
  const actor = { orgId, userId: owner };
  const [created] = await db.sql<{id:string}[]>`insert into public.experiments(org_id,profile_id,created_by,name,type,metric_focus,status)
    values(${orgId},${profileId},${owner},'Synthetic chronology','other','sales','planned') returning id`;
  const [initialEvent] = await db.sql<{id:number}[]>`insert into public.experiment_events(org_id,experiment_id,to_status,note,actor_id,created_at)
    values(${orgId},${created!.id},'planned','Created',${owner},now()+interval '1 day') returning id`;
