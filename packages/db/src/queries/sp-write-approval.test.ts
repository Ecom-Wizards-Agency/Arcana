...
describe.skipIf(!available)('authenticated SP write admission', () => {
  ...
  async function confirmation(lifetimeMs?: number, method = true): Promise<SpWriteManualApprovalRequest> {
    ...
    await database.sql`
      ...
      values (${recommendationId}, ${runId}, ${orgId}, ${profileId}, 'high_acos',
        'keyword', 'kw-1', 'bid', '0.9'::jsonb, '0.7'::jsonb, ${JSON.stringify(method ? syntheticRecommendationMethodInputs() : {})}::text::jsonb, 'accepted')
    `;
  ...
  it('refuses methodless historical previews through both shared authenticated approval functions', async () => {
    const request = await confirmation(undefined, false);
    for (const name of ['approve_sp_write_cycle', 'approve_sp_write_preview_v1']) {
      await expect(withAuthenticatedActor(database, { orgId, userId: USER }, (sql) =>
        sql.unsafe(`select app.${name}($1::uuid,$2::text)`, [request.approval.plan.planId, JSON.stringify(request.approval)])))
        .rejects.toMatchObject({ code: '55000' });
      expect(await counts(request.approval.plan.planId)).toEqual({ receipts: 0, requests: 0, wakes: 0 });
    }
  });

  it('does not start an expired approval which never had an execution request', async () => {
    const request = await confirmation(1200);
    const [approved] = await withAuthenticatedActor(database, { orgId, userId: USER }, (sql) =>
      sql<{ receipt: { executionId: string; approvalId: string; generation: string } }[]>`
        select app.approve_sp_write_cycle(${request.approval.plan.planId}::uuid, ${JSON.stringify(request.approval)}) as receipt`);
    ...
    expect(admission.kind).toBe('approved_pending_start');
    await expect(database.sql`select app.start_sp_write_execution(
      ${admission.approvalId}::uuid, ${request.approval.plan.planId}::uuid)`)
      .rejects.toMatchObject({ code: '55000' });
