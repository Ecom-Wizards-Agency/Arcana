...
import { loadExecutionSnapshot, createSpWriteRuntimeLedger, type LoadVerifiedSpWriteExecutionIdentity } from './sp-write-persistence.js';
...
export async function readSpWriteOperation(
  ...
  const root = roots[0]!;
  return loadSpWriteOperationDetail(handle, {
...
/** Package-private materialization after the caller proves exact operation read authority. */
export async function loadSpWriteOperationDetail(
  handle: { sql: QuerySql }, identity: LoadVerifiedSpWriteExecutionIdentity,
): Promise<SpWriteOperationDetail> {
  const evidence = 'begin' in handle.sql
    ? await createSpWriteRuntimeLedger({ sql: handle.sql }).loadVerifiedExecution(identity)
    : await loadExecutionSnapshot(handle.sql, identity);
  ...
  const [admission] = await handle.sql<{ queued: boolean }[]>`
    select exists(select 1 from public.sp_write_execution_requests
      where org_id = ${identity.orgId}::uuid and profile_id = ${identity.profileId}::uuid
        and execution_id = ${identity.executionId}::uuid and plan_id = ${identity.planId}::uuid
        and approval_id = ${identity.approvalId}::uuid and generation = ${identity.generation}::uuid) as queued
  `;
  ...
  const inverseRows = plan.direction === 'inverse' ? [] : await handle.sql<{ plan_id: string }[]>`
    ...
    where inverse.org_id = ${identity.orgId}::uuid and inverse.profile_id = ${identity.profileId}::uuid
      and inverse.source_execution_id = ${identity.executionId}::uuid
      and inverse.source_plan_id = ${identity.planId}::uuid and inverse.source_plan_fingerprint = ${plan.fingerprint}
      and inverse.direction = 'inverse'
    order by inverse.generated_at, inverse.plan_id
  `;
  return SpWriteOperationDetail.parse({
    operation: { executionId: identity.executionId, planId: identity.planId },
    ...
    inverses: inverseRows.map((inverse) => ({ executionId: identity.executionId, planId: inverse.plan_id })),
  });
}
