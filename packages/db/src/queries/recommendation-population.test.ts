...
describe.skipIf(!available)('counted recommendation windows', () => {
  ...
  it('returns the complete oversized export while the recommendation window reports its cap', async () => {
    ...
    // WP-253 artifact reads preserve the whole saved batch independently of UI
    // windows. Keep that contract and assert all offered proposals are returned.
    const artifact = await getExportBatch(database, { orgId, batchId: batch!.id });
    expect(artifact?.proposals).toHaveLength(count);
    expect(new Set(artifact?.proposals.map((row) => row.id)).size).toBe(count);
  });
});
