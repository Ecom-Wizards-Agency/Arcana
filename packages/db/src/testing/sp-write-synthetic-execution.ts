import { syntheticRecommendationMethodInputs } from './recommendation-method.js';
...
import { approveAndQueueSpWrite, previewSpWrite, previewSpWriteInverse, readSpWriteOperation } from '../testing/sp-write-legacy-application.js';
...
export async function seedSyntheticWriteHistory(database: TestDatabase, actor: SpWriteActor, profileId: string) {
  ...
  await database.sql`insert into public.recommendations
    ...
    values (${recommendation}, ${run.id}, ${actor.orgId}, ${profileId}, 'high_acos', 'keyword', 'kw-native-history',
      'bid', '0.9'::jsonb, '0.7'::jsonb, ${JSON.stringify(syntheticRecommendationMethodInputs())}::text::jsonb, 'accepted')`;
