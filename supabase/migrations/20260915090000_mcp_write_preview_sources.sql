...
begin
  ...
  case p_kind
    ...
    when 'row' then
      ...
      if p_value ? 'method' then v_fields := v_fields || '[["method","method"]]'::jsonb; end if;
    when 'method' then v_fields := '[["methodId","nonempty"],["methodVersion","nonempty"],["traceSha256","sha256"],["settingSourcesSha256","sha256"]]';
  ...
  for v_field in select value from jsonb_array_elements(v_fields) loop
    ...
    if right(v_kind,2) = '[]' or v_kind = 'method' then
