...
file disagree about a repository boundary, this file wins. Maintained setup and
source guides are linked from [README.md](README.md).
...
Use the package boundaries below and the public source contracts when making a
change. Planning records are not required to build, test or install this repository.
...
7. **Work on a branch** named `wp-XX-short-name`, commit in logical units, and report
   against the declared acceptance checks before merge.
...
Individually: `pnpm typecheck`, `pnpm lint`, `pnpm test`, `pnpm hygiene`, `pnpm skill-lint`.
