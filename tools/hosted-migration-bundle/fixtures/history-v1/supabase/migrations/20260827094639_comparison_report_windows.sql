alter table public.sync_schedules
  add column window_offset_days integer not null default 0,
  add constraint sync_schedules_window_offset_nonnegative
    check (window_offset_days >= 0);

comment on column public.sync_schedules.window_offset_days is
  'Whole days between yesterday and the report window end; 32 loads the comparison block before a 32-day restatement window.';

insert into public.sync_schedules
  (org_id, profile_id, job_type, report_type, variant, cadence, next_run_at,
   enabled, priority, payload, lookback_days, window_offset_days)
select org_id, profile_id, job_type, report_type, 'comparison', cadence, now(),
       enabled, priority, payload, 32, 32
  from public.sync_schedules
 where job_type = 'report.request'
   and variant = 'restatement'
...
comment on function public.enqueue_due_schedules(timestamptz) is
  'Enqueue due schedules with profile-local report windows ending yesterday, offset comparison blocks, and SQP Sunday week starts.';
