...
describe('Creative sync policy', () => {
  it('enables every sync-enabled profile without environment configuration', () => {
    expect(resolveCreativeSyncPolicy({}, true)).toEqual({ enabled: true, reason: null });
    expect(resolveCreativeSyncPolicy({}, false)).toEqual({ enabled: false, reason: 'profile_sync_disabled' });
  });

  it('disables production with a deployment reason', () => {
    expect(resolveCreativeSyncPolicy({ OPENSPELL_CREATIVE_SYNC_DISABLED: '1' }))
      .toEqual({ enabled: false, reason: 'deployment_disabled' });
    expect(resolveCreativeSyncPolicy({ OPENSPELL_CREATIVE_SYNC_DISABLED: '0' }))
      .toEqual({ enabled: true, reason: null });
    expect(() => resolveCreativeSyncPolicy({ OPENSPELL_CREATIVE_SYNC_DISABLED: 'true' }))
      .toThrow(/OPENSPELL_CREATIVE_SYNC_DISABLED/);
  });

  it('ignores lane ownership and retired deployment gates', () => {
    for (const lane of [undefined, '0', '1']) {
      expect(resolveCreativeSyncPolicy({
        OPENSPELL_EVO_REPORT_LANE_READY: lane,
        OPENSPELL_CREATIVE_SYNC_PRODUCER_READY: '0',
        OPENSPELL_CREATIVE_SYNC_PROFILE_ALLOWLIST: 'ignored',
      })).toEqual({ enabled: true, reason: null });
    }
  });
