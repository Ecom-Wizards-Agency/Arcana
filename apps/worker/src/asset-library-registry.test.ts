...
import { createDb, readAssetLibraryJobSnapshot, recordAssetLibrarySnapshot, type ClaimedJob } from '@wizard-ads/db';
...
vi.mock('@wizard-ads/db', async (original) => ({ ...await original<object>(), readAssetLibraryJobSnapshot: vi.fn(async () => null), recordAssetLibrarySnapshot: vi.fn(async () => ({ persistedRows: 0, verifiedRows: 0 })) }));
describe('asset-library production registry binding', () => {
  it('dispatches the registered job, persists the snapshot and publishes matching read counts', async () => {
    ...
    try {
      ...
      expect(readAssetLibraryJobSnapshot).toHaveBeenCalledOnce(); expect(recordAssetLibrarySnapshot).toHaveBeenCalledOnce();
