...
export async function readCollectorExport(root: string | undefined, raw: StoredCollectorExport): Promise<{ fingerprint: string; value: unknown } | null> {
  const ref = StoredCollectorExport.parse(raw);
  ...
  try {
    ...
    if (!child || child.startsWith('..') || isAbsolute(child)) throw new PermanentJobError('Export escaped configured root');
    const handle = await open(file,constants.O_RDONLY | constants.O_NOFOLLOW);
    try {
      const stat = await handle.stat();
      if (!stat.isFile() || stat.size > SPONSORED_PROMPT_IMPORT_MAX_BYTES) throw new PermanentJobError('Export is not a bounded regular file');
      ...
      if (length > SPONSORED_PROMPT_IMPORT_MAX_BYTES) throw new PermanentJobError('Export exceeds byte limit');
      ...
      catch { throw new PermanentJobError('Malformed JSON export'); }
