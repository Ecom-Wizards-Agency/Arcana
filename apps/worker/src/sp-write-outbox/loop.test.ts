import { previewSpWriteForActor, approveSpWriteForActor } from '@wizard-ads/db/sp-write-application';
...
import type { SpWriteWorkerPolicy } from './policy.js';
...
describe.skipIf(!available)('SP write worker with real ledger and fake HTTP provider', () => {
  ...
  beforeEach(async (context) => {
    ...
    for (let index = 1; index <= rows; index += 1) {
      ...
      await database.sql`insert into public.recommendations
        ...
        values (${recId}, ${run!.id}, ${orgId}, ${profileId}, 'high_acos', 'keyword', ${keywordId}, 'bid', '0.9'::jsonb, '0.7'::jsonb, ${JSON.stringify(syntheticRecommendationMethodInputs())}::text::jsonb, 'accepted')`;
    }
  ...
  function loop(overrides: { policy?: () => SpWriteWorkerPolicy; prepareProviders?: (plans: readonly SpWritePlan[]) => Promise<ReadonlyMap<string, SpWriteAdapter>> } = {}) {
    return createSpWriteOutboxLoop({ database, claimantId: 'synthetic-write-worker',
      policy: overrides.policy ?? (() => ({ dispatchEnabled, reconcileEnabled: true, profileIds: [profileId] })),
  ...
  it.each(['missing flags', 'missing profiles', 'different profile', 'different plan'] as const)(
    'makes no provider request for %s despite a valid database approval', async (scope) => {
      const policy: SpWriteWorkerPolicy = { dispatchEnabled: true, reconcileEnabled: true, profileIds: [profileId] };
      if (scope === 'missing flags') { policy.dispatchEnabled = false; policy.reconcileEnabled = false; }
      if (scope === 'missing profiles') policy.profileIds = [];
      if (scope === 'different profile') policy.profileIds = [randomUUID()];
      if (scope === 'different plan') policy.planIds = [randomUUID()];
      const worker = loop({ policy: () => policy });
      const result = await worker.tick();
      expect(['disabled', 'deferred']).toContain(result.kind);
      expect(result.attemptedCalls).toBe(0);
      expect({ credentialsPrepared, reads, puts }).toEqual({ credentialsPrepared: 0, reads: 0, puts: 0 });
  ...
  it('sends one attempt, observes it, reconciles counts and never dispatches it again', async () => {
    expect(preview.plan.counts.logicalChanges).toBe(1);
    ...
    expect(mirror).toHaveBeenCalledTimes(1);
    const a = (await detail()).snapshot.accounting;
    expect({ requested: preview.plan.counts.providerRows, admitted: a.approvedRows, attempted: puts,
      succeeded: a.providerAccepted, failed: a.providerRejected + a.refusedBeforeDispatch, observed: a.observedRequested })
      .toEqual({ requested: 1, admitted: 1, attempted: 1, succeeded: 1, failed: 0, observed: 1 });
