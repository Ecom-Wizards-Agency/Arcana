...
const screenshotDirectory = join(process.env['WP_SCRATCH'] ?? process.cwd(), 'tmp', 'screenshots', 'change-queue');
  ...
  for(const screen of ['change-queue','restore-preview','restore-batches']) {
    ...
    for(const theme of ['light','dark']) {
      ...
      if(screen!=='restore-preview') {
        ...
        await page.screenshot({path:join(screenshotDirectory,`${screen}-state-${theme}.png`)});
