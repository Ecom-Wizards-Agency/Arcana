...
import { recordEntityChanges, readRestoreExportPreview, withAuthenticatedOrgEditor } from '@wizard-ads/db';
import { createTestDatabase } from '@wizard-ads/db/testing';
import type { TestDatabase } from '@wizard-ads/db/testing';
import { POST } from '../../app/api/time-machine/restore/export/route.js';
import { POST as build } from '../../app/api/time-machine/restore/route.js';
...
describe('Time Machine restore export route', () => {
  ...
  const request = (body: Record<string, unknown>, userId = OWNER, actorOrgId = orgId) =>
    POST(
      new Request('http://localhost/api/time-machine/restore/export', {
        method: 'POST',
        headers: {
  ...
  beforeAll(async () => {
    ...
    const saved = await withAuthenticatedOrgEditor(database, { orgId, userId: OWNER }, context => readRestoreExportPreview(context, { profileId, batchId }));
    fingerprint = saved.fingerprint;
    sourceRowIds = saved.preview.rows.map(row => row.rowId);
  ...
  const validBody = () => ({
    ...
    confirmation: 'Export restore proposal (1 changes)',
    fingerprint,
  });
  ...
  it('requires the exact confirmation and current row count', async () => {
    ...
    const stale = await request({ ...validBody(), expectedRows: 2, confirmation: 'Export restore proposal (2 changes)' });
  ...
  it('builds an export-only review for a profile without write permission', async () => {
    const response = await build(new Request('http://localhost/api/time-machine/restore', {
      method: 'POST', headers: { 'content-type': 'application/json', 'x-wizard-ads-auth-bridge': BRIDGE,
        'x-wizard-ads-user-id': OWNER, 'x-wizard-ads-org-id': orgId },
      body: JSON.stringify({ requestId: '00000000-0000-4000-8000-000000000282', profileId, applyBatchId: batchId, sourceRowIds }),
    }));
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ kind: 'export_only', batchId, rows: 1 });
    const [count] = await database.sql`select count(*)::int as n from public.sp_write_plans
      where org_id=${orgId} and artifact#>>'{source,applyBatchId}'=${batchId}`;
    expect(count?.n).toBe(0);
  });

  it('refuses a changed fingerprint and a missing note before creating an export', async () => {
    expect((await request({ ...validBody(), fingerprint: 'a'.repeat(64) })).status).toBe(409);
    expect((await request({ ...validBody(), note: '' })).status).toBe(400);
    const [count] = await database.sql`select count(*)::int as n from public.apply_batches
      where org_id=${orgId} and source_batch_id=${batchId}`;
    expect(count?.n).toBe(0);
  });
