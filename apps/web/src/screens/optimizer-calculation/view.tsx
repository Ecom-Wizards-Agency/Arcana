...
export default function ScreenView({ data }: { data: ScreenData }) {
  ...
  return <OptimizerFrame title="Calculation details" subtitle={`${profile.label} · ${row.inputs.methodId ?? 'Method unavailable'} · ${row.inputs.methodVersion ?? 'Version unavailable'}`}><CalculationContent row={row} profileId={profile.id} currencyCode={profile.currencyCode} placementInputs={placementInputs} backHref={`/optimizer/review/${review.batchId}?profile=${profile.id}`} /></OptimizerFrame>;
}
