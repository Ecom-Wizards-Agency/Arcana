...
export function ExperimentDetail({
  ...
  const [inferredBatchNotes, setInferredBatchNotes] = useState<ExperimentInferredBatchNote[]>([]);
  const [note, setNote] = useState('');
  const [resultNote, setResultNote] = useState(initial.resultNote ?? '');
  const [message, setMessage] = useState('');
  const [pending, setPending] = useState(false);
  const [ready, setReady] = useState(false);
  useEffect(() => { setReady(true); const controller=new AbortController(); void fetch(`/api/experiments/${initial.id}`,{signal:controller.signal}).then(async(response)=>{if(!response.ok)return;const body=await response.json() as {events?:UiEvent[];inferredBatchNotes?:ExperimentInferredBatchNote[]};if(body.inferredBatchNotes&&!controller.signal.aborted)setInferredBatchNotes(body.inferredBatchNotes);if(body.events&&!controller.signal.aborted)setEvents((current)=>[...new Map([...current,...body.events!].map((event)=>[event.id,event])).values()]);}).catch(()=>{});return ()=>controller.abort(); }, [initial.id]);
      ...
      try {
        const response = await fetch(`/api/experiments/${experiment.id}`, {
          ...
          body: JSON.stringify({
            status: to,
            ...(note.trim() ? { note } : {}),
            ...(to === 'analyzed' && experiment.resultNote === null ? { resultNote } : {}),
          }),
        });
        ...
        if(payload.event) setEvents((current)=>current.some((event)=>event.id===payload.event!.id)?current:[...current,payload.event!]);
        setNote('');
        setMessage('Saved');
      } catch (error) {
  ...
  return (
    <main className="tl tl-experiment-detail" style={page} data-interactive={ready ? 'true' : 'false'}>
      <h1 style={{...heading,fontSize:'var(--wa-fs-base)'}} data-testid="experiment-name">{experiment.name}</h1>
      <p className="tl-note">Every status this experiment has held, who moved it and when. Append-only: an entry is never edited or removed.</p>
      <div className="tl-exp-summary">
      ...
      {experiment.resultNote && (
        <p className="tl-exp-result" data-testid="experiment-result">
          <strong>Result:</strong> {experiment.resultNote}<small> Written once at the point of analysis, alongside the status change.</small>
        </p>
      )}
      ...
      <details open={experiment.status !== 'analyzed'}><summary>Manage experiment</summary>
      {canManage && nextStatuses.length > 0 && (
        ...
        >
          ...
          {(experiment.status === 'running' || nextStatuses.includes('analyzed')) && (
            <label style={{ display: 'grid', gap: '0.25rem', fontSize: '0.8125rem' }}>
              Result note (written once when analyzed)
              <input
                value={resultNote}
                disabled={experiment.resultNote !== null}
      ...
      <details open={experiment.status !== 'analyzed'}><summary>Comparison and observed changes</summary>
      ...
      <a href={`/timeline?profile=${experiment.profileId}`}>Back to Timeline</a>
