...
export default function NamingScreen({ data }: { data: NamingData }) {
  return data.view === 'ready' ? <NamingReady data={data} /> : <CampaignPage title="Naming" subtitle="Save a convention once, reuse it, copy it between profiles. Or paste a name you already use and let it be read."><CampaignUnavailable screen="campaigns-naming" data={data} /></CampaignPage>;
}
export function NamingReady({ data, initialName = '', initiallyRead = false }: { data: Extract<NamingData, { view: 'ready' }>; initialName?: string; initiallyRead?: boolean }) {
  ...
  const matchingPreset = presets.find((preset) => JSON.stringify(preset.naming) === JSON.stringify(naming));
  ...
  return <CampaignPage title="Naming" subtitle="Save a convention once, reuse it, copy it between profiles. Or paste a name you already use and let it be read.">
    <div className="campaign-live-preview"><small>LIVE PREVIEW</small><p>{preview}</p></div>
    <section className="wa-stack"><h2 className="campaign-section-label">Reverse Builder</h2><div className="wa-actions"><Input style={{ flex: 1 }} aria-label="Existing campaign name" value={name} onChange={(event) => { setName(event.target.value); setRead(false); }} /><Button onClick={() => setRead(true)}>Read it</Button></div>
      {read && (parsed && parsed.confidence !== 'none' ? <><div className="wa-actions campaign-naming-tokens">{readTokens.map(({ token, value }) => <Badge key={token}>{tokenLabels[token] ?? token} · {value}</Badge>)}</div><p>Separator “{naming.delimiter}” · {readTokens.length} tokens · matches {matchingPreset ? `the ${matchingPreset.name} preset` : 'the current profile convention'}{parsed.confidence === 'partial' ? ' partially' : ''}. This is also what resolves a creative to its keyword; synced keyword entities are used when available.</p></> : <Notice>This name does not match the selected naming convention.</Notice>)}
    </section>
    <section className="wa-stack"><h2 className="campaign-section-label">Saved conventions</h2><Select aria-label="Copy destination profile" value={target} onChange={(event) => setTarget(event.target.value)}>{!destinations.length && <option value="">No other profiles connected</option>}{destinations.map((profile) => <option key={profile.id} value={profile.id}>{profile.label}</option>)}</Select>
      {presets.length ? presets.map((preset) => <div className="campaign-convention" key={preset.id} style={{ background: JSON.stringify(preset.naming) === JSON.stringify(data.naming) ? 'var(--wa-indigo-soft)' : 'var(--wa-surface-2)' }}><div><strong>{preset.name}</strong><p>{quantity(preset.naming.variable_order?.length ?? 0, 'token')} · used by {quantity(preset.usageCount, 'profile')}</p></div><div className="wa-actions"><Button onClick={() => { setNaming(preset.naming); setTitle(preset.name); setRead(false); }}>Use in preview</Button><Button disabled={!data.canEdit} onClick={async () => { try { await campaignRequest('/api/campaigns/naming', { action: 'copy', id: preset.id, profileId: data.profileId }); setMessage('Convention applied to this profile. Reload the builder to use it.'); } catch (error) { setMessage(error instanceof Error ? error.message : 'Apply unavailable'); } }}>Use for this profile</Button><Button disabled={!data.canEdit || !target} onClick={async () => { try { await campaignRequest('/api/campaigns/naming', { action: 'copy', id: preset.id, profileId: target }); setMessage('Convention copied to the selected profile.'); } catch (error) { setMessage(error instanceof Error ? error.message : 'Copy unavailable'); } }}>Copy to another profile</Button></div></div>) : <Notice>No saved conventions yet.</Notice>}
