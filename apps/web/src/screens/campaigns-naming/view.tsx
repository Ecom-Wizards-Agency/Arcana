...
import { Button, Input, Select, CampaignPage, Notice } from '../campaigns/ui';
...
export function NamingReady({ data, initialName = '', initiallyRead = false }: { data: Extract<NamingData, { view: 'ready' }>; initialName?: string; initiallyRead?: boolean }) {
  ...
  try { const settings = namingSettingsFromStrategy(naming); preview = generateCampaignName(settings, { goal: 'Rank', campaignType: 'SKW', matchType: 'EXACT', productName: '[product]', keywordText: '[keyword]' }, '2000-01-01'); if (read) parsed = parseCampaignName(name, settings); } catch { /* Incomplete operator input remains visibly unavailable. */ }
  ...
  return <CampaignPage title="Naming conventions">
    ...
    <section className="wa-stack"><h2>Save your convention</h2><Input aria-label="Convention name" value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Convention name" />
    ...
    <section className="wa-stack"><h2>Reverse Builder</h2><Input aria-label="Existing campaign name" value={name} onChange={(event) => { setName(event.target.value); setRead(false); }} /><Button onClick={() => setRead(true)}>Read it</Button>
      {read && (parsed && parsed.confidence !== 'none' ? <><div className="wa-actions">{readTokens.map(({ token, value }) => <span className="wa-badge" key={token}>{token} · {value}</span>)}</div><p>Separator “{naming.delimiter}” · {readTokens.length} tokens · matches the selected preset{parsed.confidence === 'partial' ? ' partially' : ''}.</p></> : <Notice>This name does not match the selected naming convention.</Notice>)}
    </section>
