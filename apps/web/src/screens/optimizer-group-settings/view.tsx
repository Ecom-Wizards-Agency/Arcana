...
export default function Screen({ data }: { data: ScreenData }) {
  return <main className={styles.page}><PageHeader title={data.view === 'ready' ? `${data.props.record.group.name} settings` : 'Group settings'} subtitle="Saved settings apply to every campaign in this group" />
    {data.view === 'gated' ? <p>{gateMessage(data.props.entry.state)}</p> : data.view === 'empty' ? <p>No profiles yet.</p> : data.view === 'missing' ? <p>Group unavailable for this profile.</p> : (() => {
      ...
      const rows = [['Group', group.name], ['Target ACOS', group.targetAcos > 0 ? `${group.targetAcos * 100}%` : 'Missing required value'], ['Bid floor / ceiling', `${group.bidFloor ?? 'Unavailable'} / ${group.bidCeiling ?? 'Unavailable'} ${profile.currencyCode}`], ['Maximum increase / decrease', `+${group.bidIncreaseCap * 100}% / −${group.bidDecreaseCap * 100}% per cycle`], ['Review schedule', `${group.reviewSchedule.weekdays.join(', ')} · ${String(workspace.reviewHour).padStart(2, '0')}:00 · ${workspace.profileTimezone}`]];
