...
import { campaignCount, formatResearchPeriod, formatResearchTimestamp, signedResearchPercent } from '../query-intelligence/research-format';
...
export function DaypartingWorkspaceView({ data, measurement, initialTab = 'schedule', initialSurface = 'main', initialEvidence = null }: { data: Ready; measurement: ReactNode; initialTab?: Tab; initialSurface?: Surface; initialEvidence?: DaypartingEvidenceSummary | null }) {
  ...
  return <main className="research"><header><h1>{title}</h1><p className="muted">{data.profile.label} · {data.profile.countryCode} · {data.profile.timezone}</p></header>
    ...
    {surface === 'evidence' ? <><h2>{name}</h2><div className="research-actions"><label className="research-field">Evidence from<input type="date" value={window.start} onChange={e => {
      ...
      setEvidence(null);
    }} /></label></div><p>{formatResearchPeriod(window)} · {data.profile.timezone} · {campaignCount(campaignIds.length || data.campaignChoices.length)}</p><table className="research-table"><thead><tr><th>Hourly evidence</th><th>Status</th></tr></thead><tbody>
      <tr><td>Hourly spend, sales and orders</td><td>{hasEvidence ? 'Available in this report' : 'Unavailable in this report'}</td></tr><tr><td>Attribution maturity</td><td>{mature ? 'Verified: settled evidence' : hasEvidence ? 'Not verified: evidence is settling or policy is unavailable' : 'Not verified'}</td></tr><tr><td>Campaign coverage</td><td>{coverage} of {campaignCount(campaignIds.length || data.campaignChoices.length)} selected</td></tr><tr><td>Schedule comparison</td><td>{hasEvidence && mature ? 'Hourly evidence can be reviewed' : 'Cannot be evaluated without mature hourly data'}</td></tr></tbody></table>
      ...
      {measurement}</> : surface === 'review' ? <><h2>{name}</h2><p>{campaignCount(campaignIds.length)} · {data.profile.timezone}</p>
        ...
        {!reviewed ? <div className="research-card warn"><h2>Review the supporting hourly report</h2><p>Check attribution maturity, campaign coverage and hourly spend before enabling this proposal.</p><ResearchAction disabled>Enable after evidence review</ResearchAction></div> : <div className="research-card"><h2>Schedule reviewed</h2><p>Reviewed {schedule?.review ? formatResearchTimestamp(schedule.review.reviewedAt, data.profile.timezone) : 'Not recorded'} · {schedule?.review ? campaignCount(schedule.review.campaignIds.length) : 'Campaign review not recorded'}</p><p>Only the reviewed schedule can be considered for a future cadence. New suggestions require another review.</p></div>}
        <p>{executionReason}</p><div className="research-actions"><ResearchAction primary disabled title={executionReason}>Yes, enable this schedule for {campaignCount(campaignIds.length)}</ResearchAction>{schedule ? <ScheduleExports schedule={schedule} /> : null}<ResearchAction onClick={() => {
          setTab('suggestions');
          setSurface('main');
        }}>Back to suggestions</ResearchAction></div></> : tab === 'suggestions' ? <><h2>Review a schedule before enabling it</h2><p>{formatResearchPeriod({ start: data.from, end: data.to })} · {data.profile.timezone} · {campaignCount(data.campaignChoices.length)}</p>
          ...
          <p>Suggestions do not change the active schedule. Review the proposed hours, evidence and limits before enabling one.</p><div className="research-actions"><ResearchAction onClick={() => void showEvidence()}>View hourly evidence</ResearchAction><ResearchInfo label="Why no suggestion?"><p>{data.workspace.maturityPolicyConfigured ? 'No eligible proposal is recorded for this window. A suggestion requires settled hourly evidence and approved model inputs.' : 'No tenant dayparting settling window is configured. A suggestion requires approved evidence and model inputs.'}</p></ResearchInfo></div></> : tab === 'results' ? <DaypartingResultsView schedule={schedule} results={schedule ? data.research.results[schedule.id] ?? null : null} currencyCode={data.profile.currencyCode} /> : <>
            <div className="research-actions"><select aria-label="Saved schedule" value={schedule?.id ?? ''} onChange={e => choose(schedules.find(s => s.id === e.target.value) ?? null)}><option value="">New schedule</option>{schedules.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select><span className="research-chip">{campaignCount(campaignIds.length)}</span><span className="research-chip" data-schedule-state={status.toLowerCase()}>{status}</span><ResearchAction onClick={() => choose(null)}>New schedule</ResearchAction></div>
            ...
            <section className="research-card" role="group" aria-labelledby="campaign-assignment-title"><h2 id="campaign-assignment-title">Campaign assignment</h2><div className="daypart-campaigns">{data.research.campaigns.map(c => <label key={c.id}><input type="checkbox" disabled={!editable} checked={campaignIds.includes(c.id)} onChange={e => {
              setCampaignIds(e.target.checked ? [...campaignIds, c.id] : campaignIds.filter(id => id !== c.id));
              setDirty(true);
              setEvidence(null);
            }} />{c.name}</label>)}</div></section>
...
export function ScheduleExecutionStatus({ schedule, onReview, onResults, initialStopOpen = false }: { schedule: DaypartingSchedule; onReview: () => void; onResults: () => void; initialStopOpen?: boolean }) {
  ...
  return <section className={`research-card ${enabled ? 'good' : ''}`}><h2>{schedule.name} is {schedule.status}</h2>{enabled ? <><p>{campaignCount(schedule.campaignIds.length)} · {schedule.timezone} · Next run at {schedule.nextRunAt ? <time dateTime={schedule.nextRunAt}>{formatResearchTimestamp(schedule.nextRunAt, schedule.timezone)}</time> : 'Unavailable from the cadence backend'}</p><p>Only the reviewed schedule runs automatically. New suggestions wait for review.</p></> : <><p>No new scheduled writes will start. A request already sent to Amazon may still finish.</p><p>Resuming requires review of the schedule, campaigns and current limits.</p></>}
...
export function DaypartingResultsView({ schedule, results, currencyCode }: { schedule: DaypartingSchedule | null; results: DaypartingResults | null; currencyCode: string }) {
  ...
  return <section className="research"><h2>Results</h2><p><strong>{schedule ? `${schedule.name} · ${campaignCount(schedule.campaignIds.length)}` : 'No enabled schedule to compare'}</strong></p><div className="research-actions"><ResearchAction aria-pressed={period === 'before'} onClick={() => setPeriod('before')}>Before enablement</ResearchAction><ResearchAction aria-pressed={period === 'after'} onClick={() => setPeriod('after')}>After enablement</ResearchAction></div>{results ? <p>{formatResearchPeriod(results[period])}</p> : null}
    <table className="research-table"><thead><tr><th>Metric</th><th>Before</th><th>After</th><th>Change</th></tr></thead><tbody>{(['spend', 'sales', 'acos', 'orders'] as const).map(metric => {
      ...
      return <tr key={metric}><td>{metric === 'acos' ? 'ACOS' : metric[0]!.toUpperCase() + metric.slice(1)}</td><td>{results?.before.complete ? format(before) : 'Pending'}</td><td>{mature ? format(after) : 'Pending'}</td><td>{mature ? before !== null && after !== null && before !== 0 ? signedResearchPercent((after - before) / before) : '—' : 'Pending until mature'}</td></tr>;
    })}</tbody></table>
    ...
    {results?.events.length ? <section><h2>Overlapping Timeline events</h2><ul>{results.events.map(event => <li key={event.id}>{event.name} · {event.kind} · {event.end ? formatResearchPeriod({ start: event.start, end: event.end }) : `${formatResearchPeriod({ start: event.start, end: event.start })} to ongoing`}</li>)}</ul></section> : null}<p>Before-and-after differences show observed performance. They do not establish that dayparting caused the change.</p></section>;
}
