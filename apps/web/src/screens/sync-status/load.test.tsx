...
const mocks = vi.hoisted(() => ({ profiles: vi.fn(), status: vi.fn(), evidence: vi.fn() }));
vi.mock('../../../app/_lib/profiles', () => ({ listProfiles: mocks.profiles }));
vi.mock('../../data/sync-status', () => ({ loadSyncStatus: mocks.status }));
vi.mock('@wizard-ads/db', () => ({ readCoreReportEvidence: mocks.evidence }));
beforeEach(() => { vi.resetAllMocks(); mocks.profiles.mockResolvedValue([profile]); mocks.status.mockResolvedValue({ profiles: [] }); mocks.evidence.mockResolvedValue([{ family: 'spAdvertisedProduct', status: 'unmeasured' }]); });
