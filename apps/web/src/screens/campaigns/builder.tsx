...
export function Builder({ context, initialStep = 'products', initialRecipe, initialSource }: {
  ...
  return <div className="campaign-builder-layout">
    <aside aria-label="Settings" className="wa-stack" style={{ background: 'var(--wa-surface)', padding: 10, position: 'sticky', top: 16 }}><span className="wa-hint">SETTINGS</span>
    ...
    <section className="wa-stack"><div className="campaign-stepper" role="tablist" aria-label="Builder steps">{[['products', '1 Products'], ['targets', '2 Play & targets'], ['review', '3 Review & create']].map(([value, label]) => <Button key={value} role="tab" aria-selected={step === value} onClick={() => setStep(value as typeof step)}>{label}</Button>)}</div>
      ...
      {step === 'targets' && <><Targets context={context} play={play} onPlay={(value) => { setPlay(value); setGroupId(''); }} text={text} onText={setText} structure={structure} onStructure={setStructure} {...(initialSource ? { initialSource } : {})} />
        <p>{quantity(keywords.length, 'keyword')} × {quantity(products.length, 'product')} = {quantity(structure === 'keyword-product' ? keywords.length * products.length : products.length, 'campaign')}{structure === 'keyword-product' ? ', 1 keyword each' : ', the keyword set in each'}</p><Button onClick={() => setStep('review')}>Review draft</Button></>}
