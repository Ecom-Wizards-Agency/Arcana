...
export function OptimizationGroupsManager({
  ...
  // Native form controls must wait for the handlers that protect the saved draft.
  const canEdit = canManage && hydrated;
  ...
  async function save(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      if (![body.assignedCampaigns, body.movedCampaigns, body.removedCampaigns].every((count) => typeof count === 'number' && Number.isSafeInteger(count) && count >= 0)
        ...
        || savedCampaigns.length !== draft.campaignIds.length
      ...
      setMessage(
        `Saved ${body.assignedCampaigns} campaign assignments` +
  ...
  return (
    <div className={`wa-stack ${styles.editor}`} data-testid="optimization-groups-manager">
      ...
      <div className="wa-opt-groups-layout">
        <aside className="wa-card wa-opt-groups-list" aria-label="Optimization groups">
          <div className="wa-section-head">
            <div>
              <span className="wa-eyebrow">Saved groups</span>
              <h2 className="wa-section-title">Optimization groups</h2>
            </div>
            {canManage ? (
              <button type="button" className="wa-btn wa-btn--sm" onClick={createNew} disabled={!canEdit}>
                New group
              </button>
            ) : null}
          </div>
          <div className="wa-opt-groups-list__items">
            {workspace.groups.length === 0 ? (
              <p className="wa-hint">No groups yet. Create a group and assign campaigns.</p>
        ...
        <form className="wa-card wa-opt-group-editor" onSubmit={save}>
          <div className="wa-section-head">
            <div>
              <span className="wa-eyebrow">{draft.id === null ? 'New group' : 'Saved group settings'}</span>
          ...
          <div className="wa-form-grid wa-form-grid--3">
            <Field label="Group name">
              <input value={draft.name} onChange={(event) => patch('name', event.target.value)} required disabled={!canEdit} />
            </Field>
            <Field label="Method">
              <select value={draft.methodId} onChange={(event) => patch('methodId', event.target.value as MethodId)} disabled={!canEdit}>
                {OPTIMIZATION_METHOD_CATALOGUE.filter((method) => MethodSelection.safeParse(method).success && ['stable', 'pilot', 'shadow'].includes(method.releaseState)).map((method) => <option key={method.id} value={method.id}>{method.displayName}{method.releaseState === 'shadow' ? ' · Shadow preview only' : ''}</option>)}
              </select>
            </Field>
            {draft.methodId === 'sp.coordinated-efficiency' ? <>
              <label>Exposure ceiling<input type="number" min="0" step="any" value={draft.exposureCeiling} onChange={(event) => patch('exposureCeiling', event.target.value)} disabled={!canEdit} /></label>
              <label>Minimum clicks per placement<input type="number" min="1" step="1" value={draft.minClicksPerPlacement} onChange={(event) => patch('minClicksPerPlacement', event.target.value)} disabled={!canEdit} /></label>
              <label>Placement evidence<select value={draft.placementEvidenceRequirements} onChange={(event) => patch('placementEvidenceRequirements', event.target.value as PlacementEvidenceRequirements)} disabled={!canEdit}>
            ...
            <Field label="Role">
              <select value={draft.role} onChange={(event) => patch('role', event.target.value as GroupDraft['role'])} required disabled={!canEdit}>
            ...
            <Field label="Prioritization">
              <select value={draft.prioritization} onChange={(event) => patch('prioritization', event.target.value as GroupDraft['prioritization'])} required disabled={!canEdit}>
            ...
            <Field label="Target ACOS" suffix="%">
              <input type="number" min="0" step="0.1" value={draft.targetAcosPercent} onChange={(event) => patch('targetAcosPercent', event.target.value)} required disabled={!canEdit} />
            </Field>
            ...
            <label className="wa-check-field">
              <input type="checkbox" checked={draft.enabled} onChange={(event) => patch('enabled', event.target.checked)} disabled={!canEdit} />
          ...
          <div className="wa-form-grid wa-form-grid--3">
            <Field label="Bid floor" optional><input type="number" min="0" step="0.01" value={draft.bidFloor} onChange={(event) => patch('bidFloor', event.target.value)} disabled={!canEdit} /></Field>
            <Field label="Bid ceiling" optional><input type="number" min="0" step="0.01" value={draft.bidCeiling} onChange={(event) => patch('bidCeiling', event.target.value)} disabled={!canEdit} /></Field>
            <span />
            <Field label="Bid increase cap" suffix="%"><input type="number" min="0" step="0.1" value={draft.bidIncreaseCapPercent} onChange={(event) => patch('bidIncreaseCapPercent', event.target.value)} required disabled={!canEdit} /></Field>
            <Field label="Bid decrease cap" suffix="%"><input type="number" min="0" step="0.1" value={draft.bidDecreaseCapPercent} onChange={(event) => patch('bidDecreaseCapPercent', event.target.value)} required disabled={!canEdit} /></Field>
            <span />
            <Field label="Placement increase cap" suffix="%"><input type="number" min="0" step="0.1" value={draft.placementIncreaseCapPercent} onChange={(event) => patch('placementIncreaseCapPercent', event.target.value)} required disabled={!canEdit} /></Field>
            <Field label="Placement decrease cap" suffix="%"><input type="number" min="0" step="0.1" value={draft.placementDecreaseCapPercent} onChange={(event) => patch('placementDecreaseCapPercent', event.target.value)} required disabled={!canEdit} /></Field>
          </div>
          ...
          <div className="wa-campaign-picker-toolbar">
            ...
            <button
              ...
              disabled={!canEdit || visibleCampaigns.length === 0}
          ...
          >
            <textarea rows={3} value={draft.exclusions} onChange={(event) => patch('exclusions', event.target.value)} disabled={!canEdit} />
          </Field>
            ...
            <div className="wa-row">
              {draft.id !== null ? (
                <button
                  ...
                  className="wa-btn"
                  onClick={runNow}
                  disabled={!canEdit || !previewReady || pending || !draft.enabled || draft.campaignIds.length === 0}
                >
                  Run group preview
                </button>
              ) : null}
              <button type="submit" className="wa-btn wa-btn--primary" disabled={!canEdit || pending}>
