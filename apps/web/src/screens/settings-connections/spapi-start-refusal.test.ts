...
import { SpApiStartDatabaseRefusal, SpApiStartField, SpApiStartRefusalClass, SpApiStartSetting, type SpApiStartRefusal } from '@wizard-ads/shared';
import { parseSpApiStartRefusal, SP_API_START_DATABASE_REFUSALS, spApiStartRefusalMessage } from './spapi-start-refusal';
...
function everyRefusal(): SpApiStartRefusal[] {
  return SpApiStartRefusalClass.options.flatMap((refusal): SpApiStartRefusal[] => {
    if (refusal === 'configuration') return SpApiStartSetting.options.map((detail) => ({ refusal, detail }));
...
describe('SP-API start refusal copy', () => {
  it('gives every admissible refusal its own message', () => {
    ...
    const expected = SpApiStartSetting.options.length + SpApiStartField.options.length + SpApiStartDatabaseRefusal.options.length + 7;
    ...
    for (const setting of SpApiStartSetting.options) expect(spApiStartRefusalMessage({ refusal: 'configuration', detail: setting })).toContain(setting);
  });
  ...
  it('parses only fixed codes from the redirect query', () => {
    ...
    for (const [error, detail] of [
      ['configuration', null], ['configuration', 'synthetic-untrusted-setting'], ['selection', 'profileId'], ['session', 'label'],
