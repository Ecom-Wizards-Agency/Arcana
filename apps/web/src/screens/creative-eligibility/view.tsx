  ...
  return <section aria-label="Asset eligibility"><LinkButton href={`/creative?${query}`} size="sm">← All creatives</LinkButton>
    ...
    <DataTable label="Asset eligibility and moderation" headers={['Asset', 'Used in', 'Moderation', 'Reason given by Amazon', 'Can it run']}>
      {assets.map((asset) => <tr key={asset.assetId}><td><div className={styles.creativeCell}><CreativeThumbnail url={asset.thumbnailUrl} name={asset.name ?? asset.assetId!} /><div><a href={creativeHref(asset.assetId!, query)}>{asset.name ?? asset.assetId}</a><small>{asset.assetId}</small></div></div></td>
        ...
        <td>{asset.eligibility?.length ? asset.eligibility.map((row, index) => <p key={index}>{row.status}<br /><small>{row.context.program} · {row.context.marketplace} · {row.identity.version} · {row.evidenceState}</small></p>) : <Missing reason="Moderation ingestion has no source" />}</td>
        ...
        <td>{asset.eligibility?.length ? asset.eligibility.map((row, index) => <p key={index}>{row.selectable ? 'Eligible in this context' : row.canRun === 'ineligible' ? 'Not eligible' : 'Not established'}<br /><small>{row.observedAt ? `Observed ${row.observedAt}` : 'No observation'}</small></p>) : <Missing reason="Eligibility has no source" />}</td>
      </tr>)}
