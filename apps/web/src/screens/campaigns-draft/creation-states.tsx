...
export function CreationConfirm({ review, checks, executor, onExport, onBack, marketplaceLabel }: {
  ...
  return <CampaignPage title="Confirm campaign creation"><section className="campaign-rationale"><h2>Create {quantity(count, 'campaign')} in Amazon</h2><p className="wa-hint">{review.profile.label} · {marketplaceLabel ?? 'Marketplace label unavailable'} · {currency} · Campaign starts paused</p></section>
...
export function CreationResult({ result, onRetry, onBack }: { result: CampaignBuilderResult; onRetry: () => void; onBack: () => void }) {
  ...
  return <CampaignPage title={complete ? 'Campaign created' : count.succeeded > 0 ? 'Campaign partially created' : 'Campaign creation unresolved'} subtitle={`${count.succeeded} of ${count.operatorApproved} resources created · Initial state paused`}>
    ...
    <details open={!complete}><summary>Resource details</summary><DetailsTable headings={['Resource', 'Requested', 'Succeeded', 'Status']} rows={result.resources.map((row) => [({ campaign: 'Campaign', ad_group: 'Ad group', product_ad: 'Product ad', keyword: 'Keyword' })[row.kind], row.requested, row.succeeded, row.status === 'created' ? `Created${row.kind === 'campaign' ? ' · Paused' : ''}` : row.status === 'failed' ? `Failed · ${row.responseCode ?? 'Reason not recorded'}` : row.status === 'pending' ? 'Pending' : 'Unknown'])} />
    <p>Requested {count.operatorApproved} · Attempted {count.attempted} · Succeeded {count.succeeded} · Failed {count.failed}</p></details>
