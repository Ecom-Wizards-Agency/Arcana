...
import { CampaignPage, Button, MoneyInput, Notice, DetailsTable, Badge, money, exactMoney, exposureEquation } from '../campaigns/ui';
...
export function BidEditor({ keyword, evidence, bounds, currency, topOfSearch, audienceAdjustment, onUse, onCancel, expanded = false, sqpMeasured = false, adType = 'Sponsored Products', match = 'Exact', placementSource = 'setting', frozenRationale, rationaleExpanded = false }: {
  ...
  adType?: string; match?: string; placementSource?: 'setting' | 'name'; frozenRationale?: string; rationaleExpanded?: boolean;
  onUse: (keyword: CampaignBuilderKeyword) => void; onCancel: () => void;
}) {
  const [rationaleOpen, setRationaleOpen] = useState(rationaleExpanded);
  ...
  const basisControl = <div className="wa-actions" aria-label="Bid basis"><small className="wa-hint">BASIS</small>{[['keyword_cpc', 'keyword CPC', !cpcAvailable], ['sqp_value', 'SQP value', true], ['manual', 'manual', false]].map(([value, label, unavailable]) => <Button key={String(value)} aria-pressed={basis === value} onClick={() => setBasis(value as typeof basis)} style={{ background: basis === value ? 'var(--wa-indigo-soft)' : 'var(--wa-surface)' }}>{label}{unavailable ? ' · unavailable' : ''}</Button>)}</div>;
  return <CampaignPage layout="bid" title={rationaleOpen ? 'The bid, and why' : 'Set starting bid'} subtitle={`${keyword.text} · ${adType} · ${match}`}>
    {rationaleOpen && basisControl}
    <div className="wa-stack campaign-bid-entry" hidden={rationaleOpen}><section className="wa-stack" aria-label="Starting bid editor"><label>Starting bid<MoneyInput currency={currency} aria-label="Starting bid amount" aria-invalid={calculation.exceeded || !calculation.inRange} type="number" value={amount} onChange={(event) => setAmount(event.target.value)} /></label>
      <p>Allowed base bid: {money(bounds.floor, currency)} to {money(bounds.ceiling, currency)}</p>
      {calculation.exceeded && <Notice kind="warn"><strong>Top-of-search exposure exceeds the limit</strong><p>Top-of-search exposure is {exactMoney(calculation.exposure, currency)}. The hard limit is {money(bounds.exposureCeiling, currency)}. Lower the bid or placement adjustment before creating the campaign.</p></Notice>}
      {!calculation.inRange && <Notice kind="bad">The entered bid must fit the allowed range and marketplace precision.</Notice>}
    </section>
    <div className="wa-actions"><Button onClick={() => setShow(!show)}>{show ? 'Hide calculation' : 'View bid calculation'}</Button><Button onClick={() => setBasis('manual')}>Enter bid manually</Button></div>
    {basisControl}
    {show && <DetailsTable headings={['Calculation', 'Value']} rows={[
      ['Reported keyword CPC', `${money(evidence?.reportedCpc, currency)} · ${cpcAvailable ? 'Source reconciled' : 'Source requires verification'}`],
      ...
      [<>Base bid formula <small className="wa-hint">· CPC suggestion</small> <small className="wa-hint">· From spend ÷ clicks</small></>, `${money(calculation.cpc, currency)} ÷ ${calculation.placementMultiplier} = ${exactMoney(cpcBase, currency)}`],
    ...
    </div><details className="campaign-bid-secondary" open={rationaleOpen} onToggle={(event) => setRationaleOpen(event.currentTarget.open)}><summary>{rationaleOpen ? 'Return to starting bid' : 'The bid, and why'}</summary><div className="wa-stack"><p className="wa-hint">{keyword.text} · {adType} · {match} · the rationale is frozen when saved, so this row still explains itself in six months.</p>
    <dl className="campaign-bid-rows">
      ...
      {row('Base bid = realised CPC ÷ (1 + adjustment) · CPC suggestion', 'Setting the base equal to the realised CPC would multiply that at top of search.', money(cpcBase, currency))}
      {row(`Bid at top of search = ${money(amount === '' ? null : Number(amount), currency)} × ${calculation.placementMultiplier}`, 'Uses the selected starting bid. Audience adjustments compound on this.', money(amount === '' ? null : Number(amount) * calculation.placementMultiplier, currency), true)}
