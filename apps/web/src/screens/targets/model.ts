import { readProductEvidence, readTargetBidContext, readSpReportEvidence, type QueryHandle, readCoreReportEvidence, readProviderEvidence, readProviderGraphEvidence } from '@wizard-ads/db';
import { AdProduct, ProviderGraphScope, providerGraphIdentityKey, type ProviderGraphAssociation } from '@wizard-ads/shared';
...
export async function loadTarget360(handle: QueryHandle, args: { orgId: string; profileId: string; targetId: string; from: string; to: string }) {
  ...
  const adProduct = payload.target.adProduct;
  const providerEvidence = adProduct === 'SP' || adProduct === 'SB' || adProduct === 'SD'
    ? await readProviderEvidence(handle, { orgId: args.orgId, profileId: args.profileId, consumer: 'targets', entityId: args.targetId, adProduct, entityType: payload.target.targetKind === 'keyword' ? 'keyword' : 'target' })
    : undefined;
  const [ranks, performance, changes, bidContext, searchEvidence, graph] = await Promise.all([
  ...
  const catalogueAdProduct = AdProduct.safeParse(payload.target.adProduct);
  ...
  const shelf = !catalogueAdProduct.success ? [] : (await Promise.all(marketplaces.map(({marketplace_id}) => readProductEvidence(handle,{scope:{orgId:args.orgId,profileId:args.profileId,marketplaceId:marketplace_id},asins,adProduct:catalogueAdProduct.data,staleAfter})))).flat();
  return { ...(providerEvidence ? { providerEvidence } : {}), ...({ searchEvidence } as { searchEvidence?: Awaited<ReturnType<typeof loadTargetSearchEvidence>> }), payload, ranks, performance, changes, bidContext, graph, shelf, ...(coreEvidence.some((item) => item.rowCount > 0) ? { coreEvidence } : {}), profileId: args.profileId, currencyCode };
}
