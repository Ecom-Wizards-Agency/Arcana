import { readProductEvidence, readTargetBidContext, readSpReportEvidence, type QueryHandle } from '@wizard-ads/db';
...
export async function loadTarget360(handle: QueryHandle, args: { orgId: string; profileId: string; targetId: string; from: string; to: string }) {
  ...
  return { ...(providerEvidence ? { providerEvidence } : {}), ...({ searchEvidence } as { searchEvidence?: Awaited<ReturnType<typeof loadTargetSearchEvidence>> }), payload, ranks, performance, changes, bidContext, shelf, ...(coreEvidence.some((item) => item.rowCount > 0) ? { coreEvidence } : {}), profileId: args.profileId, currencyCode };
}
