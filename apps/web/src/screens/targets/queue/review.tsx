...
export function QueueReview({ change, back }: { change: QueuedBidChange; back: string }) {
  const [approved, setApproved] = useState(change.approvedAt !== null);
  ...
  const approve = async () => {
    ...
    try {
      ...
      if (!response.ok || result.status !== 'approved') throw new Error(result.error ?? 'Approval could not be confirmed. Reload before trying again.');
      setApproved(true);
  ...
  return <main className={styles.tabContent} data-state={approved ? 'approved' : 'queued'}><h1>Review queued change</h1>
    ...
    <div className={styles.actions}><a className="wa-btn" href={target}>View target and limits</a><button className="wa-btn wa-btn--primary" disabled={!pass || approved || busy} onClick={() => void approve()}>{busy ? 'Approving…' : 'Approve after checks pass'}</button></div>
    {approved ? <p role="status">Approved. Approval was recorded; no Amazon bid was changed.</p> : <p>Queueing and approval do not change an Amazon bid.</p>}{error ? <p role="alert">{error}</p> : null}
  </main>;
}
