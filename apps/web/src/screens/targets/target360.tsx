...
export function Target360({ model, currencyCode, back, savedView, onClose, showLimits = false }: { model: Target360Model; currencyCode: string; back: string; savedView: string | null; onClose?: () => void; showLimits?: boolean }) {
  ...
  return <article className={styles.root} style={onClose ? { margin: 0 } : undefined} data-state={queued ? 'queued' : rankBlocked ? 'rank-gated' : hasSeries ? 'populated' : 'empty-series'}>
    ...
    <AbaEvidencePanel evidence={model.searchEvidence?.aba} selectedAsin={model.searchEvidence?.asin ?? ''} query={model.payload.target.targeting} />
      ...
      {tab === 'Shelf' ? <section aria-label="Product evidence"><h2>Shelf</h2>{model.shelf.length===0?<p>Product evidence is missing for this target&apos;s advertised ASINs.</p>:<table><thead><tr><th>ASIN</th><th>Marketplace</th><th>Evidence</th><th>Title</th><th>Availability</th><th>Eligibility</th><th>Provider reason</th><th>Observed</th></tr></thead><tbody>{model.shelf.map((item)=><tr key={`${item.scope.marketplaceId}:${item.asin}`}><td>{item.asin}</td><td>{item.scope.marketplaceId}</td><td>{item.availability}</td><td>{item.metadata?.title.state==='returned'?item.metadata.title.value:'Not measured'}</td><td>{item.metadata?.availability.state==='returned'?item.metadata.availability.value:'Not measured'}</td><td>{item.eligibility?.verdict??'unknown'}</td><td>{item.eligibility?.reasons.map((reason)=>reason.code).join(', ')||'Not measured'}</td><td>{item.metadata?.provenance.providerObservedAt??item.metadata?.provenance.retrievedAt??item.eligibility?.provenance.providerObservedAt??item.eligibility?.provenance.retrievedAt??'Not measured'}</td></tr>)}</tbody></table>}<p>Product eligibility is product and ad-program evidence. It does not establish asset moderation approval or permission to create a campaign.</p></section> : null}
