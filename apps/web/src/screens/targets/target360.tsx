...
import { describeTargeting, normalizeQueuedBidOverride, parseGridView, placementLabel, serializeGridView, targetKindLabel, type GridSavedView, type ListingFieldObservation } from '@wizard-ads/shared';
...
export function Target360({ model, currencyCode, back, savedView, onClose, showLimits = false }: { model: Target360Model; currencyCode: string; back: string; savedView: string | null; onClose?: () => void; showLimits?: boolean }) {
  ...
  // One reading of the target for every place it is named (V19): keyword text, or the expression in words.
  const described = describeTargeting({ targeting: model.payload.target.targeting, targetKind: model.payload.target.targetKind, matchType: model.payload.target.matchType });
  const kindLabel = targetKindLabel(model.payload.target.targetKind, model.payload.target.targeting);
  ...
  return <article className={styles.root} style={onClose ? { margin: 0 } : undefined} data-state={queued ? 'queued' : rankBlocked ? 'rank-gated' : hasSeries ? 'populated' : 'empty-series'}>
    ...
    <header className={styles.header}>
      <div className={styles.breadcrumb}><a href={returnTo}>Back to grid</a> / Targets / {described.label}</div>
      <div className={styles.titleRow}><div><h1>{described.label}</h1><p className={styles.meta}>{described.type ?? 'Match type not measured'} · {kindLabel ?? 'Kind not measured'} · {model.payload.target.state ?? 'State not measured'} | {model.payload.target.adProduct} | {model.payload.target.campaignName}</p></div>
    ...
    <div className={styles.signals}><div className={styles.rank}><span className={styles.rankMark}>R</span><div><strong>{rank?.organicRank == null ? 'Not measured' : `#${rank.organicRank}`}</strong><p className={styles.meta}>daily · {model.ranks.length} observations{rank ? ` · to ${new Intl.DateTimeFormat('en-GB', { day: 'numeric', month: 'short', timeZone: 'UTC' }).format(new Date(rank.date))}` : ''}</p></div></div>
      <div className={styles.notice}>{share ? `T · ${percent(share.topOfSearchShare)} on ${share.date}. I · P not measured` : 'T · I · P not measured'}<small>{share ? '' : 'Top-of-search share: column empty. '}SQP shares require a matched query, ASIN and counted report period.</small></div></div>
    ...
    {tab === 'Corridor' ? <section id="target-Corridor" role="tabpanel" className={styles.corridor}>
      ...
      {tab === 'Rank' ? <><section aria-label="Rank observations"><h2>Rank observations</h2>{model.ranks.length === 0 ? <p>No rank observations measured for this keyword and profile in this period.</p> : <table><thead><tr><th>Date</th><th>ASIN</th><th>Organic rank</th><th>Sponsored rank</th></tr></thead><tbody>{newestFirst(model.ranks).map((r,i) => <tr key={i}><td>{r.date}</td><td>{r.asin}</td><td>{r.organicRank ?? 'Not measured'}</td><td>{r.sponsoredRank ?? 'Not measured'}</td></tr>)}</tbody></table>}</section>
    ...
    <AbaEvidencePanel evidence={model.searchEvidence?.aba} selectedAsin={model.searchEvidence?.asin ?? ''} query={model.payload.target.targeting} />
      ...
      {tab === 'Changes' ? <><h2>Changes</h2>{model.changes.length === 0 ? <p>No entity changes recorded in this period.</p> : <table><thead><tr><th>Date</th><th>Field</th><th>Before</th><th>After</th><th>Source</th></tr></thead><tbody>{newestFirst(model.changes).map((r) => <tr key={r.id}><td>{r.date}</td><td>{r.field}</td><td>{r.oldValue ?? 'Not measured'}</td><td>{r.newValue ?? 'Not measured'}</td><td>{r.source}</td></tr>)}</tbody></table>}</> : null}
      {tab === 'Performance' ? <><h2>Performance</h2>{model.performance.length === 0 ? <p>No target facts measured in this period.</p> : <table><thead><tr><th>Date</th><th>Impressions</th><th>Clicks</th><th>Spend</th><th>Sales</th><th>Orders</th><th>ACOS</th><th>Top-of-search share</th></tr></thead><tbody>{newestFirst(model.performance).map((r) => <tr key={r.date}><td>{r.date}</td><td>{r.impressions ?? 'Not measured'}</td><td>{r.clicks ?? 'Not measured'}</td><td>{money(r.spend)}</td><td>{money(r.sales)}</td><td>{r.orders ?? 'Not measured'}</td><td>{percent(r.acos)}</td><td>{percent(r.topOfSearchShare)}</td></tr>)}</tbody></table>}</> : null}
    </section>}
    ...
    {(view.compare?.length ?? 0) > 0 ? <section aria-label="Compare targets" className={styles.compare}><h2>Compare targets ({view.compare!.length}/4)</h2>{view.compare!.map((p) => {
      ...
      return <section key={key}><button className="wa-btn" onClick={() => update({ ...view, compare: view.compare!.filter((c) => c !== p) })}>Remove {other ? describeTargeting({ targeting: other.payload.target.targeting, targetKind: other.payload.target.targetKind }).label : p.targetId}</button>{other ? <BidCorridorChart title={describeTargeting({ targeting: other.payload.target.targeting, targetKind: other.payload.target.targetKind }).label} ariaLabel={`Compare ${describeTargeting({ targeting: other.payload.target.targeting, targetKind: other.payload.target.targetKind }).label}`} currencyCode={other.currencyCode} points={other.payload.points} /> : <p aria-busy="true">Loading comparison {p.targetId}…</p>}</section>;
    })}</section> : null}
