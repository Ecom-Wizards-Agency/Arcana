...
import { ResearchAction, researchMoney, researchPercent, researchMutation } from '../query-intelligence/research-ui';
...
export function BrandLens({ data, initialTab = 'setup' }: { data: Extract<BrandLensData, { view: 'ready' }>; initialTab?: 'setup' | 'review' | 'overview' | 'exclusions' }) {
  ...
  return <main className="research"><header><h1>Brand lens</h1><p className="muted">{data.profile.label} · {data.period.start} to {data.period.end}</p></header><nav className="research-tabs" aria-label="Brand lens sections">{(['setup', 'review', 'overview', 'exclusions'] as const).map(t => <ResearchAction key={t} primary={tab === t} aria-pressed={tab === t} onClick={() => setTab(t)}>{t === 'setup' ? 'Word setup' : t === 'review' ? 'Review classifications' : t === 'overview' ? 'Ad performance by bucket' : 'Campaign exclusions'}</ResearchAction>)}</nav>
    ...
    {tab === 'setup' ? <><h2>Brand lens — setup</h2><p>Classifies your own keywords into branded, competitor and generic. The model proposes tokens; rules do the classifying; you own the boundary.</p>
      ...
      <strong>{rows.length} keywords will be classified</strong><p className="muted">Classification is rule-based and deterministic. Re-running with the same tokens gives the same answer.</p><div><ResearchAction primary onClick={() => setTab('review')}>Classify keywords</ResearchAction></div></> : null}
    {tab === 'review' ? <><h2>Review classifications</h2><p>{rows.length} keywords · 3 buckets · every row can be overridden, and an override sticks through the next run.</p><div className="research-table-scroll"><table className="research-table"><thead><tr><th>Keyword</th><th>Proposed</th><th>Matched on</th><th>Spend</th><th>ACOS</th><th>Override</th></tr></thead><tbody>{rows.map(r => <tr key={r.id}><td>{r.keyword}</td><td><span className={`research-chip ${r.bucket}`}>{names[r.bucket]}</span></td><td className={r.matchedOn === 'no token matched' ? 'muted' : ''}>{r.matchedOn}</td><td>{money(r.spend)}</td><td>{researchPercent(r.acos)}</td><td>{r.decision ? <span className={`research-chip ${r.decision}`}>{r.decision}</span> : null}<select aria-label={`Override ${r.keyword}`} disabled={busy} value="" onChange={e => {
      const [bucket, decision] = e.target.value.split(':');
      if (bucket && decision) void decide(r.keyword, bucket as BrandLensBucket, decision as BrandLensDecision);
    }}><option value="">change</option><option value={`${r.proposed}:kept`}>Keep proposed</option><option value={`${r.bucket}:confirmed`}>Confirm {names[r.bucket]}</option>{BRAND_BUCKETS.map(b => <option key={b} value={`${b}:changed`}>{names[b]}</option>)}</select>{r.decision ? <ResearchAction disabled={busy} onClick={() => void mutate('/api/brand-lens/overrides', {
      action: 'remove',
      profileId: data.profile.id,
      keyword: r.keyword
    }).then(result => {
      if (result) setOverrides(overrides.filter(o => o.normalizedKeyword !== normalizeResearchQuery(r.keyword)));
    })}>Remove override</ResearchAction> : null}</td></tr>)}</tbody></table></div><p className="muted">Core and head both belong to Generic here. Explicit query exclusions and unreviewed tokens remain visible for judgement; classification does not exclude a campaign.</p></> : null}
    {tab === 'overview' ? <><h2>Ad performance by bucket</h2><table className="research-table"><thead><tr><th>Bucket</th><th>Share of spend</th><th>Spend</th><th>Sales</th><th>ACOS</th><th>Clicks</th><th>CPC</th></tr></thead><tbody>{buckets.map(b => <tr key={b.bucket}><td><span className={`research-chip ${b.bucket}`}>{names[b.bucket]}</span></td><td>{researchPercent(b.share)}<div className="research-share"><span style={{ width: `${(b.share ?? 0) * 100}%` }} /></div></td><td>{money(b.spend)}</td><td>{money(b.sales)}</td><td>{researchPercent(b.acos)}</td><td>{b.clicks ?? '—'}</td><td>{money(b.cpc)}</td></tr>)}</tbody></table>
      <div className="research-card info" data-testid="brand-insight">{measured ? <p>{researchPercent(branded.share)} of ad spend is defending your own brand name, at {researchPercent(branded.acos)} ACOS. Generic runs at {researchPercent(generic.acos)}; blended ACOS is {researchPercent(totalSpend!==null&&totalSales!==null&&totalSales>0?totalSpend/totalSales:null)}. {buckets[1]!.spend === 0 ? 'Competitor spend is zero. Review whether that matches the agreed strategy.' : buckets[1]!.spend === null ? 'Competitor performance is not measured in this window.' : `Competitor spend is ${money(buckets[1]!.spend)}.`}</p> : <p>No keyword performance is measured in this window.</p>}</div><p className="muted">Branded ROAS cannot tell you whether this spend protected demand or captured sales that would have happened anyway. Answering that needs a holdout, not a bucket.</p></> : null}
    {tab === 'exclusions' ? <><h2>Campaign exclusions</h2><p>Keep chosen campaigns out of optimization so an automatic bid change does not undo the agreed brand strategy.</p><div className="research-card">Classifying a term does not block it. Campaign exclusions control eligibility for new optimization proposals.</div><table className="research-table"><thead><tr><th>Campaign</th><th>Bucket it serves</th><th>Optimization</th><th>Why</th></tr></thead><tbody>{campaigns.map(c => {
      const bucket = dominantBrandBucket(rows, c.id);
      return <tr key={c.id}><td>{c.name}</td><td>{bucket ? names[bucket] : 'Unclassified'}</td><td>{c.groupId ? <button type="button" role="switch" aria-checked={c.excluded} aria-label={`Exclude ${c.name}`} className={`research-chip ${c.excluded ? 'confirmed' : ''}`} disabled={busy} onClick={() => void exclude(c.id, !c.excluded)}>{c.excluded ? 'Excluded' : 'Included'}</button> : <span>Assign to a group first</span>}</td><td>{c.groupId ? `${c.groupRole ?? 'Assigned group'} · ${c.excluded ? 'excluded from new optimization proposals' : 'eligible under group settings'}` : 'No optimization group'}</td></tr>;
    })}</tbody></table></> : null}
  </main>;
}
function TokenGroup({ title, description, kinds, entries, busy, onWord }: { title: string; description: string; kinds: QueryVocabularyKind[]; entries: QueryVocabularyEntry[]; busy: boolean; onWord: (action: 'add' | 'remove' | 'approve', kind: QueryVocabularyKind, value: string, id?: string) => Promise<void> }) {
  const [value, setValue] = useState(''), [kind, setKind] = useState(kinds[0]!);
  const own = entries.filter(e => kinds.includes(e.kind)), proposed = own.filter(e => e.source === 'ai_suggestion');
  return <section className="research-card"><div className="research-actions" style={{ justifyContent: 'space-between' }}><h2>{title}</h2><span className="muted">{proposed.length ? `model proposed ${proposed.length} · you kept ${proposed.filter(e => e.approved).length}` : 'No model proposals yet'}</span></div><p className="muted">{description}</p><div className="research-actions">{own.map(e => <span className={`research-chip ${title === 'Brand tokens' ? 'branded' : title === 'Competitor tokens' ? 'competitor' : ''}`} key={e.id}>{e.value}{!e.approved ? <button disabled={busy} onClick={() => void onWord('approve', e.kind, e.value, e.id)}>Approve</button> : null}<button aria-label={`Remove ${e.value}`} disabled={busy} onClick={() => void onWord('remove', e.kind, e.value, e.id)}>×</button></span>)}<label className="research-field">Add to {title}<input value={value} onChange={e => setValue(e.target.value)} /></label>{kinds.length > 1 ? <select aria-label="Brand token kind" value={kind} onChange={e => setKind(e.target.value as QueryVocabularyKind)}><option value="own_brand_term">Brand term</option><option value="own_brand_alias">Misspelling or alias</option></select> : null}<ResearchAction disabled={busy || !value.trim()} onClick={() => void onWord('add', kind, value).then(() => setValue(''))}>+ add</ResearchAction></div></section>;
}
