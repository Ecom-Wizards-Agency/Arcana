...
import { amazonConnectionsEnabled, secureCookies, stateSigningKey } from '../env';
...
} from '../auth/security-authorization';
...
import type { StateFailure } from '../oauth/state';


const SETTINGS = '/settings/connections';

export async function receiveAmazonConsent(request: Request, params = new URL(request.url).searchParams): Promise<Response> {
  const secure = secureCookies();

  const cookieNonce = readCookie(request, nonceCookieName(secure));
  let verification;
  try { verification = verifyState(stateSigningKey(), params.get('state'), cookieNonce); }
