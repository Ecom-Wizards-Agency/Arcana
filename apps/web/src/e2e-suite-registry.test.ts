...
const EXPECTED_REGISTRY = [
  ...
  ['auth', 'authenticated-dev', 'playwright.auth.config.ts', 'auth', ['dashboard.spec.ts', 'grid.spec.ts'], 5],
...
describe('web E2E suite registry', () => {
  ...
  it('conserves all 76 logical test cases and resolves every dispatch entry', () => {
    expect(E2E_SUITE_DEFINITIONS.reduce((total, suite) => total + suite.expectedTests, 0)).toBe(76);
