...
export default defineConfig({
  ...authConfig,
  testMatch: e2eTestMatch('auth-guards-anonymous-a'),
  outputDir: './node_modules/.cache/playwright/auth-guards-anonymous-a',
  reporter: withE2ESummaryReporter(process.env['CI']
    ? [
        ...
        [
          'html',
          {
            outputFolder: './node_modules/.cache/playwright/reports/auth-guards-anonymous-a',
            open: 'never',
          },
        ],
      ]
    : [['list']]),
  projects: [{ name: 'auth-guards-anonymous-a', use: { ...devices['Desktop Chrome'] } }],
});
